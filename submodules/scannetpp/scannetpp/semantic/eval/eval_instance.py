...
import numpy as np
import scannetpp.semantic.utils.instance_utils as instance_utils
...
def evaluate_matches(matches, label_info, eval_opts):
    ...
    ap = np.zeros(
        (len(dist_threshes), len(label_info.class_labels), len(overlaps)), float
    )
    ...
    for di, (min_region_size, distance_thresh, distance_conf) in enumerate(
        ...
        for oi, overlap_th in enumerate(tqdm(overlaps, desc="overlap", leave=False)):
            ...
            for li, label_name in enumerate(
                tqdm(label_info.class_labels, desc="class", leave=False)
            ):
                # over all scenes
                y_true = np.empty(0)
                y_score = np.empty(0)
...
def assign_instances_for_scan(
    pred_file, gt_file, preds_dir, ignore_mask, label_info, eval_opts
):
    try:
    ...
    gt_instances = instance_utils.get_instances(
        gt_ids,
...
def verify_pred_files(pred_files, label_info):
    ...
    for pred_file in tqdm(pred_files, desc="check_pred_files"):
        ...
        with open(pred_file) as f:
            lines = [l.strip().split() for l in f.readlines()]  # noqa: E741

        for line_ndx, line in enumerate(
            tqdm(lines, desc="check_mask_file", leave=False)
        ):
            ...
            assert int(line[1]) in range(len(label_info.all_class_ids)), (
                f"Invalid class ID in {pred_file} at line {line_ndx}"
            )
            # check if the confidence is between 0 and 1
            assert 0 <= float(line[2]) <= 1, (
                f"Invalid confidence in {pred_file} at line {line_ndx}"
            )
...
def eval_instance(
    ...
    # evaluate
    results = evaluate(
        pred_files, gt_files, preds_dir, data_root, label_info, eval_opts
    )
