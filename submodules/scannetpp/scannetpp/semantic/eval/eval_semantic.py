...
def eval_semantic(
    ...
    for scene_id in tqdm(scene_list):
        assert (Path(pred_dir) / f"{scene_id}.txt").is_file(), (
            f"Pred file {scene_id}.txt not found"
        )

    # create one confmat for each k
    confmats = {
        k: ConfMat(num_classes, top_k_pred=k, ignore_label=ignore_label)
        for k in top_k_pred
    }
    ...
    for scene_id in tqdm(scene_list):
        ...
        pred = np.loadtxt(
            Path(pred_dir) / f"{scene_id}.txt", dtype=np.int32, delimiter=","
        )

        assert pred.shape[0] == gt.shape[0], (
            f"Prediction for {scene_id} does not match #GT vertices: pred={pred.shape}, GT={gt.shape}"
        )
        ...
        if not eval_against_gt:
            ...
            assert pred.min() >= 0, (
                f"Prediction for {scene_id} contains negative labels: {pred.min()}"
            )
        # assert max of preds is  <= (num_classes-1)
        assert pred.max() <= (num_classes - 1), (
            f"Prediction for {scene_id} contains labels > {num_classes - 1}: {pred.max()}"
        )
        ...
        assert len(pred.shape) == 1 or pred.shape[1] == max_k, (
            f"Prediction shape {pred.shape} not supported"
        )
        ...
        with warnings.catch_warnings():
            ...
            ignore_vtx = torch.LongTensor(
                np.loadtxt(scene.scan_mesh_mask_path, dtype=np.int32)
            )
