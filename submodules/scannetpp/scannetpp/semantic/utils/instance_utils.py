...
def read_instance_prediction_file(filename, pred_path):
    ...
    for line in lines:
        parts = line.split(" ")
        assert len(parts) == 3, (
            f"Invalid instance prediction file {filename}. Expected (per line): [rel path prediction] [label id prediction] [confidence prediction]"
        )
        assert not os.path.isabs(parts[0]), (
            f"Invalid instance prediction file {filename}. First entry in line must be a relative path"
        )
