...
def compute_embeddings(
    ...
    for label in tqdm(unique_labels, desc=mask_level, total=unique_labels.shape[0]):
        ...
        for s in stats:
            s.score = (s.visible_count / s.label_count) * (s.area / max_area)
            
        candidates = [s for s in stats if s.score > 0]
        if not candidates:
            lang_img_embeds.append(
                torch.zeros(lang_model.embed_dim, dtype=lang_model.dtype)
            )
        ...
        if len(pool) < topk: pool = candidates

        for _ in range(topk - 1):
            if len(selected) >= len(candidates): break
