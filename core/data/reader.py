...
def sample(x: list[T], num: int = -1, nth: int = -1, dist: str = "uniform") -> list[T]:
    ...
    - dist: Distribution type, either "first" to take the first num items or "uniform" to sample uniformly (default), or "random".
    """
    if num > 0:
        if dist == "first":
            x = x[:num]
            ...
            if len(x) > num:
                indices = sorted(random.sample(range(len(x)), num))
                x = [x[i] for i in indices]

    if nth > 1:
        if dist == "random":
            num_to_sample = max(1, len(x) // nth)
            if len(x) > num_to_sample:
                indices = sorted(random.sample(range(len(x)), num_to_sample))
                x = [x[i] for i in indices]
        else:
            x = x[::nth]
