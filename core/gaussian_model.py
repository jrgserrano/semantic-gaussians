...
from core.utils.scene_utils import BasicPointCloud, init_parameters
from core.utils.sh_utils import RGB2SH
...
class GaussianModel:
    def __init__(self) -> None:
        ...
        # SE(3) unique keyframe IDs
        # self.unique_kfIDs = torch.empty(0, dtype=torch.int32)
    ...
    def load_state_dict(self, state_dict: dict):
        ...
        self.unique_kfIDs = state_dict.get("unique_kfIDs", torch.zeros(self._xyz.shape[0], dtype=torch.int32, device=self._xyz.device))
...
def create_from_pcd(
    ...
    dist2 = torch.clamp(
        distCUDA2(fused_point_cloud).float().to(device), min=0.0000001, max=0.01
    )
    scales = torch.log(torch.sqrt(dist2))[..., None].repeat(1, 3)
    rots = torch.zeros((fused_point_cloud.shape[0], 4), device=device)
    rots[:, 0] = 1

    # Revert to standard 3DGS 0.5 opacity for better visibility before densification kicks in.
    ...
    model.unique_kfIDs = torch.zeros(fused_point_cloud.shape[0], dtype=torch.int32, device=device)
    return model
...
def create_from_ply(
    ...
    model.unique_kfIDs = torch.zeros(xyz.shape[0], dtype=torch.int32, device=device)
    return model
...
def save_ply(model: GaussianModel, path: Path):
    ...
    f_dc_tensor = model._features_dc.detach()
    if f_dc_tensor.dim() == 3:
        f_dc = f_dc_tensor.transpose(1, 2).flatten(start_dim=1).contiguous().cpu().numpy()
    else:
        f_dc = f_dc_tensor.flatten(start_dim=1).contiguous().cpu().numpy()
        
    f_rest_tensor = model._features_rest.detach()
    if f_rest_tensor.dim() == 3:
        f_rest = f_rest_tensor.transpose(1, 2).flatten(start_dim=1).contiguous().cpu().numpy()
    else:
        f_rest = f_rest_tensor.flatten(start_dim=1).contiguous().cpu().numpy()
