...
from core.cluster.hdbscan import hdbscan_clustering
from core.data import load_scene_info
from core.eval.scannetpp.export_predictions import export_scenes_predictions
from core.gaussian_model import create_from_pcd, create_from_ply, save_gaussians
from core.gaussian_optimizer import GaussianOptimizer

from core.gaussian_renderer import render
from core.language import LanguageModel
from core.language.embed import embed
from core.viewer.server import ViewerServer
from core.semantic.descriptions import compute_descriptions
...
from core.params import ModelParams, OptimizationParams, PipeParams
from core.scene import Scene
from core.utils.general_utils import seed_everything
from core.utils.scene_utils import save_scene_info
...
def training(
    ...
    if opt_params.use_fisher_sampler:
        sampler = FisherCameraSampler(all_cameras)
    else:
        from core.utils.train_utils import train_cameras
        sampler_gen = train_cameras(scene)
        
        class DummySampler:
            def sample(self):
                return next(sampler_gen)
            def update_score(self, uid, surprise_value):
                pass
                
        sampler = DummySampler()

    # Initialize live visualization server
    viewer_server = None
    if not no_server:
        viewer_server = ViewerServer(port=port)
        viewer_server.config_data = {
            "is_training": True,
            "has_labels": False,
            "has_bboxes": False
        }
    ...
    for iteration in range(first_iter, opt_params.iterations + 1):
        ...
        if render_depth and rendered_depth is not None and has_depth:
            gt_depth = viewpoint_cam.original_depth.to(device).squeeze()
            ren_depth = rendered_depth.squeeze()
            valid_mask = gt_depth > 0
            if valid_mask.any():
                depth_loss = l1_loss(ren_depth[valid_mask], gt_depth[valid_mask])
        ...
        with torch.no_grad():
            ...
            if iteration % 10 == 0:
                ...
                for key in ["psnr_test", "psnr_train", "ssim_test", "ssim_train", "lpips_test", "lpips_train", "feature_instability"]:
            ...
            if viewer_server is not None and (iteration % 100 == 0 or iteration == opt_params.iterations):
                try:
                    with torch.no_grad():
                        ...
                        if getattr(viewer_server, "render_mode", "rgb") == "semantic" and gaussians.get_features is not None:
                            from core.utils.vis_utils import pca
                            features = gaussians.get_features.detach().cpu().numpy()
                            colors = pca(features, n_components=3, normalize=True).astype(np.float32)
                        else:
                        ...
                        # Encode current ground-truth and rendered image to JPEG for the viewer
                        def tensor_to_jpeg_bytes(tensor):
                            if tensor is None:
                                return b""
                            img = (tensor.detach().cpu().numpy().transpose(1, 2, 0) * 255).clip(0, 255).astype(np.uint8)
                            img_pil = Image.fromarray(img)
                            buf = io.BytesIO()
                            img_pil.save(buf, format="JPEG", quality=70)
                            return buf.getvalue()

                        def depth_to_jpeg_bytes(depth_tensor):
                            ...
                            depth_rgb = np.stack([depth_rgb, depth_rgb, depth_rgb], axis=-1)
                            img_pil = Image.fromarray(depth_rgb)
                            buf = io.BytesIO()
                            img_pil.save(buf, format="JPEG", quality=70)
                            return buf.getvalue()
                            
                        live_image_bytes = tensor_to_jpeg_bytes(viewpoint_cam.original_image[:3])
                        rendered_image_bytes = tensor_to_jpeg_bytes(render_pkg.render[:3])
                        
                        live_depth_bytes = depth_to_jpeg_bytes(viewpoint_cam.original_depth) if hasattr(viewpoint_cam, "original_depth") else b""
                        rendered_depth_bytes = depth_to_jpeg_bytes(render_pkg.depth) if hasattr(render_pkg, "depth") else b""
                        
                        # Provide the first camera pose so the viewer initializes there
                        w2c = all_cameras[0].world_view_transform.T.detach().cpu().numpy()
                        c2w = np.linalg.inv(w2c)
                        init_pose = c2w.T.flatten().tolist()
                        
                        viewer_server.update_data(
                            splat_data.tobytes(), 
                            poses_data={"current_pose": None, "keyframes": [init_pose]},
                            live_image_bytes=live_image_bytes,
                            rendered_image_bytes=rendered_image_bytes,
                            live_depth_bytes=live_depth_bytes,
                            rendered_depth_bytes=rendered_depth_bytes
                        )
                except Exception as e:
                    print(f"[ WARNING ] Live visualization update failed: {e}")
            ...
            if opt_params.adaptive_pruning:
                # 100 -> 500
                ...
                curr_densify_interval = opt_params.densification_interval
                curr_grad_threshold = opt_params.densify_grad_threshold
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Training of an Semantic Gaussians model")
    ...
    parser.add_argument("--no-server", action="store_true", help="Disable the viewer server")
    parser.add_argument(
