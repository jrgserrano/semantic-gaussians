...
export class SplatRenderer {
    ...
    initWebGL() {
        ...
        gl.vertexAttribIPointer(indexAttributeLocation, 1, gl.INT, false, 0, 0);
        gl.vertexAttribDivisor(indexAttributeLocation, 1);
    ...
    resize(width, height) {
        ...
        this.gl.uniform2fv(this.u_viewport, new Float32Array([width, height]));
    }

    setProjection(projectionMatrix, fx, fy) {
        this.gl.uniformMatrix4fv(this.u_projection, false, new Float32Array(projectionMatrix));
        this.gl.uniform2fv(this.u_focal, new Float32Array([fx, fy]));
    }

    updateSplats(buffer, vertexCount) {
        this.worker.postMessage({ buffer, vertexCount });
    }

    render(viewProjMatrix, viewMatrix) {
        if (this.vertexCount === 0) return;
        
        this.worker.postMessage({ view: new Float32Array(viewProjMatrix) });
        
        this.gl.uniformMatrix4fv(this.u_view, false, new Float32Array(viewMatrix));
