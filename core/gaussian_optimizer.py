...
from core.params import OptimizationParams
...
class GaussianOptimizer:
    ...
    def add_importance_stats(self, opacity_grad_rgb: torch.Tensor | None = None, opacity_grad_feat: torch.Tensor | None = None, opt: OptimizationParams | None = None):
        """
        Accumulates the weighted importance score psi_i.
    ...
    def prune_points(self, mask: torch.BoolTensor):
        ...
        if hasattr(self.model, "unique_kfIDs"):
            self.model.unique_kfIDs = self.model.unique_kfIDs[valid_points_mask]
        if hasattr(self.model, "trackable_mask"):
            self.model.trackable_mask = self.model.trackable_mask[valid_points_mask]
    ...
    def densification_postfix(
        ...
        new_features: torch.Tensor | None = None,
        new_kfIDs: torch.Tensor | None = None,
        new_trackable_mask: torch.Tensor | None = None,
    ):
        try:
            d = {
                "xyz": new_xyz,
            ...
            # append kf_ids
            if new_kfIDs is not None:
                self.model.unique_kfIDs = torch.cat((self.model.unique_kfIDs, new_kfIDs.to(self.model.unique_kfIDs.device)), dim=0)
            else:
                self.model.unique_kfIDs = torch.cat((self.model.unique_kfIDs, torch.full((new_xyz.shape[0],), -1, dtype=torch.int32, device=self.model.unique_kfIDs.device)), dim=0)
                
            if hasattr(self.model, "trackable_mask"):
                if new_trackable_mask is not None:
                    self.model.trackable_mask = torch.cat((self.model.trackable_mask, new_trackable_mask.to(self.model.trackable_mask.device)), dim=0)
                else:
                    self.model.trackable_mask = torch.cat((self.model.trackable_mask, torch.zeros((new_xyz.shape[0],), dtype=torch.bool, device=self.model.trackable_mask.device)), dim=0)
    ...
    def densify_and_clone(
        ...
        new_unique_kfIDs = (
            self.model.unique_kfIDs[selected_pts_mask]
            if hasattr(self.model, "unique_kfIDs")
            else None
        )
        new_trackable_mask = (
            self.model.trackable_mask[selected_pts_mask]
            if hasattr(self.model, "trackable_mask")
            else None
        )
    ...
    def densify_and_split(
        ...
        new_unique_kfIDs = (
            self.model.unique_kfIDs[selected_pts_mask].repeat(N)
            if hasattr(self.model, "unique_kfIDs")
            else None
        )
        new_trackable_mask = (
            self.model.trackable_mask[selected_pts_mask].repeat(N)
            if hasattr(self.model, "trackable_mask")
            else None
        )
